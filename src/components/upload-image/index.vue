<template>
  <div class="flex flex-wrap">
    <template v-if="limit == 1">
      <div class="rounded cursor-pointer overflow-hidden relative border border-solid border-color image-wrap mr-[10px]"
           :style="style">
...
import {computed, reactive, watch, toRaw, ref, onMounted} from 'vue'
...
const setValue = () => {
  ...
  previewImageList = toRaw(images.data).map((url: string) => {
    return img(url)
  })
}
...
onMounted(() => {
  Sortable.create(imageContainer.value, {
    animation: 150,
    onEnd: (evt) => {
      const movedItem = images.data.splice(evt.oldIndex, 1)[0];

      images.data.splice(evt.newIndex, 0, movedItem);
      emit('change', value.value);
