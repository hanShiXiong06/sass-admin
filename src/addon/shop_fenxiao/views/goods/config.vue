      ...
      <el-form class="page-form" :model="formData" label-width="200px" ref="formRef" v-if="Object.keys(detail).length">
        ...
        <el-card class="card !border-none" shadow="never">
          <el-form-item :label="t('goodsImage')">
            <el-image v-if="detail.goods_info.goods_cover_thumb_small" class="w-[98px] h-[98px]"
                      :src="img(detail.goods_info.goods_cover_thumb_small)" fit="contain">
              <template #error>
                    ...
                    <template #default="{ row, $index }">
                      <div class="h-[62px] border-box py-[15px]">
                        <el-form-item :prop="'skuList[' + $index + '].calculate_price'" :rules="[{
                          ...
                          <el-input v-model="row.calculate_price" clearable class="input-width mr-[10px]"
                ...
                <el-table-column :label="t('skuName')" min-width="120">
                  <template #default="{ row }">
                    <p class="h-[124px] flex items-center px-[16px] border-box"
                       ...
                       v-for="(item, index) in formData.skuList" :key="index">
                      {{ item.sku_name || detail.goods_info.goods_name }}</p>
                  </template>
                </el-table-column>
                    ...
                    <template v-for="(item, index) in formData.skuList" :key="index">
                      <div class="h-[62px] border-box py-[15px] px-[16px]">
                        <el-form-item v-if="formData.fenxiao_rule[item.sku_id][row.level_id]"
                          ...
                          <el-input
                              v-model="formData.fenxiao_rule[item.sku_id][row.level_id].one_rate"
                              clearable
                      ...
                      <div class="h-[62px] border-box py-[15px] px-[16px]"
                        ...
                        <el-form-item v-if="formData.fenxiao_rule[item.sku_id][row.level_id]"
                          ...
                          <el-input
                              v-model="formData.fenxiao_rule[item.sku_id][row.level_id].one_money"
                              clearable
                    ...
                    <template v-for="(item, index) in formData.skuList" :key="index">
                      <div class="h-[62px] border-box py-[15px] px-[16px]">
                        <el-form-item v-if="formData.fenxiao_rule[item.sku_id][row.level_id]"
                          ...
                          <el-input
                              v-model="formData.fenxiao_rule[item.sku_id][row.level_id].two_rate"
                              clearable
                      ...
                      <div class="h-[62px] border-box py-[15px] px-[16px]"
                        ...
                        <el-form-item v-if="formData.fenxiao_rule[item.sku_id][row.level_id]"
                          ...
                          <el-input
                              v-model="formData.fenxiao_rule[item.sku_id][row.level_id].two_money"
                              clearable
...
const getDetail = (id: number) => {
  ...
  getFenxiaoGoodsInfo(id).then((res: any) => {
    ...
    formData.value.is_fenxiao = 1 //res.data.goods_info.is_set_fenxiao
    ...
    res.data.goods_info.skuList.forEach((item, index) => {
      ...
      if (!formData.value.fenxiao_rule[item.sku_id]) {
        ...
        res.data.rule.forEach((subItem, subIndex) => {
          if (res.data.goods_info.site_id !== 100005) {
            ...
            formData.value.fenxiao_rule[item.sku_id][subItem.level_id] = cloneDeep(subItem);
            formData.value.fenxiao_rule[item.sku_id][subItem.level_id].one_rate = "";
            formData.value.fenxiao_rule[item.sku_id][subItem.level_id].two_rate = "";
            let one_money;
            const price = res.data.goods_info.skuList[subIndex].price;
            if (price < 3000) {
              one_money = "100";
            ...
            formData.value.fenxiao_rule[item.sku_id][subItem.level_id].one_money = one_money;
            formData.value.fenxiao_rule[item.sku_id][subItem.level_id].two_money = "0";
          }
        });
      }
