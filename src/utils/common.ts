...
export function setThemeColor(color: string, mode: string = "light"): void {
  ...
  Object.keys(colors[mode]).forEach((key) => {
    useCssVar("--el-color-primary" + "-" + key, null).value =
      colorFunction.convert(`color(${color} ${colors[mode][key]})`);
  });
}
...
export function img(path: string): string {
  return isUrl(path)
    ? path
    : `${import.meta.env.VITE_IMG_DOMAIN || location.origin}/${path}`;
}
...
export function urlToRouteRaw(url: string) {
  ...
  param &&
    param.split("&").forEach((str: string) => {
 ...
 * @param where
 */
export function setTablePageStorage(
  page: any = 1,
  limit: any = 10,
  where: any = {}
) {
